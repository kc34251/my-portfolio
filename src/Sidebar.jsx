...
export default function Sidebar({ children, size = "default", position = "left", showSocial = true }) {
  ...
  const positionClasses = {
    ...
    bottom: "bottom-0 left-0 w-full h-auto portrait:flex-row portrait:justify-between portrait:items-center landscape:hidden"
  };

  return (
    <aside className={`fixed ${sizeClasses[size]} ${positionClasses[position]} bg-transparent p-6 flex items-start text-left`}>
      <Link to="/gallery" className={`text-3xl font-bold block leading-tight ${position === 'bottom' ? 'portrait:mr-8 mb-8 portrait:mb-12' : 'mb-6'}`}>
        CHRIS<br />
        MF<br />
        CHEN
      </Link>
      {!children && (
        <p className="text-lg italic text-gray-700 mb-8">creative endeavors</p>
      )}

      {children ? (
        <div className={`w-full ${position === 'bottom' ? 'portrait:w-auto portrait:space-y-6 portrait:text-right space-y-4' : 'space-y-4'}`}>
          {children}
        </div>
      ) : (
      ...
      <div className={`mt-8 flex justify-center space-x-4 ${showSocial ? '' : 'hidden'}`}>
        <a
          href="https://www.instagram.com/chrismfchenarts"
          target="_blank"
